...
$("html").append(`
  ...
  <button style="width:100%;border-radius: 8px;font-size:100%" id='exportNotes'>Export Notes</button>
  </div>
...
document.getElementById('exportNotes').addEventListener('click', ()=>{
  if(!toggleExport){
      var exportHTML =''
      for(foo_en=1;foo_en<note_count;foo_en++){
        exportHTML+="<h2> Note:"+foo_en+"</h2>";
        exportHTML+= WBJS_HTML[foo_en];
        exportHTML+="<hr>";

      }
      $("body").append ( '                                           \
           <div id="noteHTMLParent" style="left: 10%; height: 80%; \
       ...
       display: inline-block; max-width: 80%;overflow-x: hidden;"><div id="notesHTML"></div> </div>                                                       \
       ');
     $("#notesHTML").html(exportHTML.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')  );
     // $('#notesHTML').delay(15000).fadeOut('slow');
     toggleExport= true;

  }
  else{
    $("#noteHTMLParent").remove();
    toggleExport=false;
